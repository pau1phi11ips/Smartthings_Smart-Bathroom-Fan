...
definition(
    ...
    namespace: "pau1phi11ips",
    author: "Brian Keifer & Paul Phillips",
    description: "Turns on/off a switch (for an exhaust fan) based on humidity.",
    category: "My Apps",
    iconUrl: "https://raw.githubusercontent.com/pau1phi11ips/SmartThings/master/icons/smart_fan.png",
    iconX2Url: "https://raw.githubusercontent.com/pau1phi11ips/SmartThings/master/icons/smart_fan@2x.png",
    iconX3Url: "https://raw.githubusercontent.com/pau1phi11ips/SmartThings/master/icons/smart_fan@3x.png"
)
...
def prefPage() {
    ...
    def inputIgnoreModes = [
        name:       "ignoreModes",
        type:       "mode",
        title:      "Ignore in these Modes",
        multiple:   true,
        required:   false
    ]
    dynamicPage(name: "prefPage", title: "Preferences", uninstall: true, install: true) {
        ...
        section("Modes") {
            input "ignoreModes", "mode", title: "Ignore in these Modes", multiple: true, required:   false
        }
        section("Enable?") {
...
def eventHandler(evt) {
    ...
    if (eventValue >= (rollingAverage + humidityHigh) && fanSwitch.currentSwitch == "off" && enabled) {
        if (enabled) {
        	if (settings.ignoreModes?.contains(location.mode)) {
            	log("Humidity (${eventValue}) is more than ${humidityHigh}% above rolling average (${rollingAverage.round(1)}%).  Not turning on fan set to ignore mode: ${location.mode}")
            } else {
